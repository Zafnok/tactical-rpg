...
movement ([ADR-0006](../../docs/adr/0006-input-actions-and-virtual-cursor.md)).
Must feel snappy.
...
**In:** cursor state and drawing, movement/jump/clamp, camera follow, cursor
...
1. `Cursor { pos: Pos, blink_t: f32 }` in `BattleScreen`. `CursorLeft/…` move 1
   tile, `CursorJump…` move 5, clamped to map bounds. Camera follows (0401).
...
5. Help bar reflects context (e.g. over own ready unit: `f select · s info · Tab next unit`).
6. On battle start, cursor starts on the first player lord (or first player unit).

## Acceptance criteria

- [ ] Holding `l` moves the cursor with the repeat timing from the keymap; `Shift+l` jumps 5.
...
- Harness: `"l l l"` → cursor x+3; `hold("l", 1.0)` → expected position from repeat maths; clamp at edges; `Shift+j` jump; `Tab` cycling order and wrap; camera origin after long moves.
